...
{
    const amgclControls& controls = amgclControls::New
    (
    ...
    if (firsttimein)
    {
        ...
        buildMat(ptrs, cols, vals, rhss, AMGxs, AMGrs);
        forAll(psi, pi)
        {
            AMGxs[pi] = psi[pi];
        }
    }
    amgclLinearSolverContext::Solver::params prm;
    ...
    if(ctx.caching.needsMatrixUpdate())
    {
        ...
        updateMat(ptrs, cols, vals, rhss, AMGxs, fvm, source);
    }
    ...
    auto Acl = std::tie(An, ptrs, cols, vals);
    amgclLinearSolverContext::Solver AMGsolve(Acl, prm);
    size_t AMGiters;
    ...
    // solve the matrix and return the AMGx
...
) const
{
    ...
    label num_nonzero = n + matrix_.lduAddr().lowerAddr().size() + matrix_.lduAddr().upperAddr().size();
...
) const
{
    ...
    label vali = 0, AMGrow = 0, AMGcol = 0;
